...
class Poll extends Component {
    ...
    getTimeRemaining = (poll) => {
        ...
        if (days > 0) {
            timeRemaining = days + " 天剩余";
        } else if (hours > 0) {
            timeRemaining = hours + " 小时剩余"
        } else if (minutes > 0) {
            timeRemaining = minutes + " 分钟剩余"
        } else if (seconds > 0) {
            timeRemaining = seconds + " 秒剩余"
        } else {
            timeRemaining = "剩余时间小于1秒"
        }
            ...
            <div className="poll-content">
                <div className="poll-header">
                    <div className="poll-creator-info">
                        <Link className="creator-link" to={`/users/${this.props.poll.createdBy.name}`}>
